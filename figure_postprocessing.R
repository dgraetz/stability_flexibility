...
AR_betw_RT_range <- AR_betw_Error@data$estimate %>% range()
AC_betw_RT_range <- AC_betw_Error@data$estimate %>% range()

betw_RT_range <- c(min(c(AR_betw_RT_range[1], AC_betw_RT_range[1])), max(c(AR_betw_RT_range[2], AC_betw_RT_range[2])))
betw_RT_range[1] <- -0.1
betw_RT_range[2] <- betw_RT_range[2] + 0
...
AC_betw_RT <- AC_betw_RT + 
  ...
  coord_cartesian(ylim = c(0, betw_RT_range[2]), xlim = c(1, 4.5))+ 
  theme(
