...
alt_run_agg <- alt_run %>%
  group_by(id, is_bivalent, was_prev_bivalent, cycle) %>%
  summarize(RT = mean(time[filter_correct == 0], na.rm = TRUE)) %>%
  normWS(s = "id", dv = "RT") %>%
  mutate(RT = DV_n) %>%
  group_by(is_bivalent, was_prev_bivalent, cycle) %>%
...
ggplot(alt_run_agg, aes(x = cycle, y = RT_m, color = is_bivalent, linetype = was_prev_bivalent, group = interaction(is_bivalent, was_prev_bivalent)))+
