...
alt_run <- alt_run %>% 
  ...
  mutate(cycle2 = ifelse(cycle > 4, cycle - 4, cycle), lerror = lag(error, 1), ltrial = lag(trial, 1), lrt = lag(rt, 1)) %>% 
  group_by(id, block) %>% #coding our valence variable
  mutate(valence = if_else(dimshape == 4 | dimcolor == 4,1,2), lvalence = lag(valence, 4)) %>% #univalent = 1, bivalalent = 2; lvalence l = 1 group so for me 4 
  ungroup()
alt_run

#compute z-score by subject by cycle + valence
